...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray<AVMetadataMachineReadableCodeObject *> *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    ...
    if (self.isQRCode == YES) {
        result = [metadataObjects.firstObject stringValue];
    } else {
        for (int i = 0; i < metadataObjects.count; i++) {
            AVMetadataMachineReadableCodeObject *codeObj = [metadataObjects objectAtIndex:i];
            NSString *tmpString = codeObj.stringValue;
